    ...
    if result.returncode != 0:
        ...
        if "quota" in error_message.lower() or "limit" in error_message.lower():
            raise GeminiQuotaExceededError(
                f"Gemini API quota exceeded: {error_message}"
            )
        else:
...
class GeminiProcessor:
    ...
    def format_transcript(self, transcript_path: Path):
        ...
        if not prompt_path.exists():
            logger.error(
                "The prompt to format the text block is not at the given path. Exiting."
            )
        ...
        logger.info(
            f"--- Processing transcript into paragraphs: {transcript_path.name} using Gemini ---"
        )
        ...
        for attempt_number in range(1, max_attempts + 1):
            ...
            try:
                ...
                if response_content:
                    ...
                    if 0.98 <= ratio <= 1.02:
                        logger.info(
                            f"Successfully formatted text block with {ratio:.2f} ratio in word count"
                        )
                        ...
                        logger.warning(
                            f"Word count ratio failed. Ratio was {ratio:.2f}. Expected between 0.98 and 1.02."
                        )
                else:
                    logger.warning(
                        f"Gemini returned no content for {transcript_path.name}."
                    )
                ...
                logger.error(
                    f"Error on attempt {attempt_number} for {transcript_path.name}: {e}"
                )
...
class OllamaProcessor:
    ...
    def _get_prompts_to_process(self):
        prompt_files = {
            ...
            "main_passage": "extract_main_passage.txt",
            "title": "title_prompt.txt",
            "applications": "applications.txt",
            "characters": "characters_mentioned.txt",
            "issues": "issues_raised.txt",
            "key_topics": "key_topics.txt",
            "quotes": "quotes.txt",
    ...
    def _execute_prompt(self, transcript_text, prompt_path: Path):
        ...
        try:
            ...
            logger.error(
                "Could not decode JSON from Ollama. Raw response: %s", response_json
            )
            return None

    def _execute_generative_consolidation(
        self, transcript_text: str, original_prompt_path: Path, category_name: str
    ):
        consolidation_prompt_path = self.PROMPTS_DIR / "consolidation_prompt.txt"
        if not consolidation_prompt_path.exists():
            raise FileNotFoundError(
                f"Consolidation prompt file not found: {consolidation_prompt_path}"
            )

        responses = [
            self._execute_prompt(transcript_text, original_prompt_path)
            for _ in range(5)
        ]
        ...
        if not responses:
            logger.error(
                "No responses generated for '%s'. Cannot consolidate.", category_name
            )
            return {"best": None, "alternatives": []}

        consolidation_template = consolidation_prompt_path.read_text(encoding="utf-8")
        response_placeholders = {
            f"RESPONSE_{i+1}": responses[i] if i < len(responses) else ""
            for i in range(5)
        }
        ...
        return (
            {"best": responses[0], "alternatives": responses[1:]}
            if responses
            else {"best": None, "alternatives": []}
        )
    ...
    def generate_metadata(self, transcript_path: Path, save_to_json=True):
        ...
        logger.info(
            "--- Processing transcript with Ollama: %s ---", transcript_path.name
        )
            ...
            try:
                if use_consolidation:
                    response_content = self._execute_generative_consolidation(
                        transcript_text, prompt_path, name
                    )
                else:
                    response_content = self._execute_prompt(
                        transcript_text, prompt_path
                    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if FORMATTED_TEXT_PATH.exists():
            ...
            logger.warning(
                "Formatted transcript not found. Skipping OllamaProcessor test."
            )
